...
void afisareTabela(HashTable ht) {
    ...
    for (int i = 0; i < ht.dim; i++) {
        ...
        if (ht.tabela[i] != NULL) {
            ...
            printf("\Tranzactiile de pe pozitia %d sunt: \n", i);
            afisareLista(ht.tabela[i]);

        }
        else {

            printf("\nPe pozitia %d nu avem tranzactii: \n", i);
...
void dezalocareLista(Nod** cap) {
    ...
    while (p) {
        ...
        Nod* temp = p;
        p = p->next;
        if (temp->tranzactie.denumire != NULL) {

            free(temp->tranzactie.denumire);
...
int main() {
    ...
    HashTable ht = citireDinFisier("tranzactii.txt", 6);
    afisareTabela(ht);
    ...
    printf("\n ==========  Lista tranzactii dupa dezalocare  ============ \n");
    afisareTabela(ht);
